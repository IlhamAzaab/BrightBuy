...
const Orders = () => {
  ...
  const fetchOrders = useCallback(async (status) => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn('Unexpected orders payload, coercing to empty array:', data);
        setOrders([]);
      }
    } catch (err) {
  ...
  const markCompleted = async (orderId) => {
    try {
      const res = await fetch(`http://localhost:9000/api/orders/${orderId}/status`, {
        method: "PUT",
        ...
        <div className="flex gap-4 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded transition-colors ${
              tab === "pending" ? "bg-blue-600 text-white" : "bg-white shadow border"
            }`}
          >
            Pending
          </button>
          <button
            ...
            className={`px-4 py-2 rounded transition-colors ${
              tab === "completed" ? "bg-blue-600 text-white" : "bg-white shadow border"
            }`}
          >
            Completed
          </button>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {orders.map((o) => {
              ...
              const displayStatus = isCompleted ? 'completed' : (o.deliveryStatus || 'pending');
              return (
                <div
                  key={o.id}
                ...
                >
                  {/* Accent bar */}
                  <div className={`h-1 w-full ${o.status === 'completed' ? 'bg-green-500' : 'bg-amber-400'} opacity-80`}></div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      <div className="space-y-1">
                        <h2 className="font-semibold text-lg tracking-tight flex items-center gap-2">
                          ...
                          {o.status === 'completed' && (
                            <span className="text-[10px] uppercase tracking-wide font-medium text-green-600 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">Done</span>
                          )}
                        </h2>
                        <div className="flex flex-wrap gap-4 text-xs text-gray-500">
                          <div>Placed: <span className="font-medium text-gray-600">{placed}</span></div>
                          <div>Total Items: <span className="font-medium text-gray-600">{o.items.length}</span></div>
                          {!isCompleted && o.estimatedDelivery && (
                            <div className="flex items-center gap-1">
                              ETA: <span className="font-medium text-gray-600">{new Date(o.estimatedDelivery).toLocaleDateString()}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-2 min-w-[140px]">
                        <span className={`text-xs font-medium px-3 py-1 rounded-full border ${statusBadge} capitalize`}>{displayStatus}</span>
                        <div className="text-right">
                          <p className="text-[11px] uppercase tracking-wide text-gray-400">Total</p>
                          <p className="text-xl font-semibold text-gray-800">${o.total}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-1 border border-gray-100 rounded-lg overflow-hidden">
                      ...
                      <ul className="divide-y divide-gray-100">
                        {o.items.map((item, idx) => (
                          <li key={idx} className="grid grid-cols-12 px-4 py-2 text-sm md:items-center gap-2 md:gap-0">
                            <div className="col-span-12 md:col-span-5 font-medium text-gray-700 flex items-start">
                              <span className="line-clamp-2">{item.product}</span>
                            </div>
                            <div className="col-span-6 md:col-span-3 text-gray-500 text-xs md:text-sm">{item.variant}</div>
                            <div className="col-span-3 md:col-span-2 md:text-center text-gray-600 font-medium">{item.qty}</div>
                            <div className="col-span-3 md:col-span-2 text-right text-gray-700 font-semibold">${item.subtotal}</div>
                          </li>
                        ))}
                      </ul>
                    </div>
