...
export default function Checkout() {
  ...
  const loadAddress = async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/user/address`, { withCredentials: true });
      if (res.data?.address) {
...
const loadEstimate = async () => {
  try {
    const res = await axios.get(`${API_BASE}/api/estimate`, { withCredentials: true });
...
useEffect(() => {
  ...
  const handleSaveAddress = async () => {
    try {
      await axios.put(`${API_BASE}/api/user/address`, { address, city }, { withCredentials: true });
...
<div className="border p-4 rounded-lg shadow-sm mb-6">
  <h2 className="text-lg font-semibold text-gray-800 mb-3">Delivery Method</h2>
        ...
        <div className="border rounded-lg p-6 shadow-md sticky top-24 bg-white">
          ...
          <p className="flex justify-between text-gray-700 mb-4">
            ...
            <span className="font-bold text-green-700">
              {formatPrice(
                cartData.items.reduce((sum, i) => sum + i.Price * i.Quantity, 0)
              )}
            </span>
          </p>
